...
function mountAvatar(target, options = {}) {
  ...
  const playbackRate = Math.max(0.25, Math.min(3, Number(options.animationSpeed) || 1));
  let maximumFps = Math.max(1, Math.min(60, Number(options.fps) || 60));
  const motionStrength = options.reducedMotion ? 0.35 : 1;
  const scaledMs = duration => duration / playbackRate;
  ...
  let ambientStrength = motionStrength;
  ...
  const render = (time = performance.now()) => {
    const eyeElapsed = (time - eyeAmbientStartedAt) * playbackRate;
    const bodyElapsed = (time - bodyAmbientStartedAt) * playbackRate;
  ...
  const tick = time => {
    frameRequest = null;
    const hadTransition = Boolean(transitionState);
    const hadBlink = Boolean(blinkState);
    if (transitionState) {
      ...
      ambientStrength = clamp01(eased) * motionStrength;
      ...
      if (linear >= 1) {
        ...
        ambientStrength = motionStrength;
      }
    }
    if (blinkState) {
    ...
    const visualActive = Boolean(transitionState || blinkState || ambientActive);
    const reachedFinalFrame = (hadTransition && !transitionState) || (hadBlink && !blinkState);
    if (reachedFinalFrame || time - lastRenderedFrame >= 1000 / maximumFps) {
      render(time);
      lastRenderedFrame = time;
    }
    if (visualActive) frameRequest = requestAnimationFrame(tick);
  };
  const requestTick = () => {
  ...
  const scheduleBlink = (animation, delay) => {
    ...
    blinkTimer = setTimeout(() => {
      blinkDueAt = null;
      blinkState = { startedAt: performance.now(), durationMs: scaledMs(animation.blink.durationMs) };
      requestTick();
      const range = animation.blink.maxIntervalMs - animation.blink.minIntervalMs;
      scheduleBlink(animation, scaledMs(animation.blink.durationMs + animation.blink.minIntervalMs + Math.random() * range));
    }, delay);
  };
  ...
  const runStep = animation => {
    ...
    animateTo(step.expressionId, scaledMs(step.transitionMs), step.transition);
    const duration = scaledMs(step.transitionMs + step.holdMs);
  ...
  const api = {
    ...
    setFps(nextFps) {
      maximumFps = Math.max(1, Math.min(60, Number(nextFps) || 60));
      return api;
    },
    play(animationName) {
      ...
      if (animationName === currentAnimation && paused) {
        ...
        scheduleBlink(
          ...
          pausedBlinkDelay || scaledMs(DATA.animations[currentAnimation].blink.minIntervalMs)
        );
        if (AvatarProceduralEngine.hasAmbientMotion(currentPose.expression)) requestTick();
      ...
      scheduleBlink(DATA.animations[currentAnimation], scaledMs(DATA.animations[currentAnimation].blink.initialDelayMs));
      return api;
    },
    pause() {
      const now = performance.now();
