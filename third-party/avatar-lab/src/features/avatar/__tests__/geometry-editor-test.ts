...
describe('body node editor geometry', () => {
  ...
  it('normalizes tiny imported sphere precision drift without flattening real ellipsoids', () => {
    const rotated = poseFromExpression({
      ...defaultExpression,
      headX: 17,
      headY: 34,
      headZ: -21,
    ...
    expect(renderAvatar(rotated, nearSphere, 1).headPath).toBe(
      renderAvatar(poseFromExpression(defaultExpression), nearSphere, 1).headPath
    )
    expect(renderAvatar(rotated, ellipsoid, 1).headPath).not.toBe(
      renderAvatar(poseFromExpression(defaultExpression), ellipsoid, 1).headPath
    )
  })
})
