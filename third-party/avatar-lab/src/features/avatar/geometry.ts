...
const projectedEllipsoidPath = (pose: AvatarPose, surface: SurfaceConfig) => {
  const dimensions = [surface.width, surface.height, surface.depth]
  ...
  const radii: Point3 = isSphere
    ...
    : [surface.width / 2, surface.height / 2, surface.depth / 2]
  const ellipse = projectedEllipsoid(pose, radii)
  if (ellipse && isSphere) {
